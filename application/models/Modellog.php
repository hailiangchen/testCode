...
class Modellog extends CI_Model
{
    ...
    function AddLog($options = array()){

        $arr=array('addtime','record','recordcontent','username');

        if(!$this->_required($arr, $options)) return false;
        // Add where col to adds
        $qualificationArray = array('addtime','record','recordcontent','username');
    ...
    function UpdateLog($options = array())
    {
        // required values
        ...
        $qualificationArray = array('id','username','addtime','record','recordcontent');
        foreach ($qualificationArray as $qualifier) {
            if (isset($options[$qualifier]))
