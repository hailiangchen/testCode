...
class Modeluserinfo extends CI_Model
{
    ...
    function AddUserinfo($options = array()){

        $arr=array();

        if(!$this->_required($arr, $options)) return false;
        // Add where col to adds
        $qualificationArray = array('realname','usercard','bankno','bankname','cardtype','cardno','username','usermobile');
    ...
    function UpdateUserinfo($options = array())
    {
        // required values
        ...
        $qualificationArray = array('id','realname','usercard','bankno','bankname','cardtype','cardno','username','usermobile');
        foreach ($qualificationArray as $qualifier) {
            if (isset($options[$qualifier]))
    ...
    function DeleteUserinfo($options = array())
    {
        // required values
        if(!$this->_required(array('id'), $options)) return false;
